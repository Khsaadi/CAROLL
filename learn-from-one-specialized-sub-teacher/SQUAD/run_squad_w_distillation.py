...
""" This is the modified script from `examples/question-answering/run_squad.py` (as of 2020, January 8th) with an additional and optional step of distillation."""
...
MODEL_CLASSES = {
    ...
    "bert_embedder": (BertConfig, BertModel, BertTokenizer),
...
def train(args, train_dataset, model, tokenizer, teacher=None):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            loss, start_logits_stu, end_logits_stu, outputs_stu_hidden_states = outputs[0], outputs[1], outputs[2], outputs[3]
            # Distillation loss
            if teacher is not None:
                ...
                with torch.no_grad():
                    ...
                    last_hidden_states, pooled_output, outputs_tea_hidden_states = outputs_tea[0], outputs_tea[1], outputs_tea[2]

                loss = args.alpha_squad * loss
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        for i, example_index in enumerate(example_indices):
            ...
            output = [to_list(outputs[j][i]) for j in range(len(outputs))]
...
def main():
    ...
    if args.teacher_type is not None:
        ...
        assert args.alpha_ce + args.alpha_cos + args.alpha_corr > 0.0
        assert args.alpha_ce + args.alpha_squad + args.alpha_cos + args.alpha_corr > 0.0
